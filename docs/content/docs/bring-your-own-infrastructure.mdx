...
<div className="border-fd-border bg-fd-secondary/50 mb-4 flex items-center gap-2 rounded-md border px-3 py-2 text-sm">
  <span className="text-fd-muted-foreground">Contact us to get started:</span>
  <CopyEmailButton email="founders@emdash.sh" />
</div>
...
>
  ...
  <img src="/media/docker-logo.svg" alt="Docker" style={{ height: '40px', objectFit: 'contain' }} />
</div>
...
| `EMDASH_TASK_ID`     | The task identifier (same value passed to the provision script)         |
...
| **Working directory** | Project root                           |
| **Env vars**          | `EMDASH_INSTANCE_ID`, `EMDASH_TASK_ID` |
| **Stdout**            | (ignored)                              |
| **Stderr**            | Log lines                              |
| **Exit code**         | 0 = success, non-zero = failure        |
| **Timeout**           | 2 minutes                              |
