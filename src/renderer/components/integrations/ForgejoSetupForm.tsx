...
const ForgejoSetupForm: React.FC<Props> = ({
  instanceUrl,
  token,
  onChange,
  onSubmit,
      ...
      {!hideHeader && (
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-md border border-border/70 bg-muted/40 px-2 py-0.5 text-xs font-medium">
            <AgentLogo logo={forgejoLogoSvg} alt="Forgejo" className="h-3.5 w-3.5" />
            Forgejo setup
          </span>
          <span className="text-xs text-muted-foreground">
            Connect to Forgejo with a personal access token.
          </span>
        </div>
      )}
      ...
      <div className="mt-2 rounded-md border border-dashed border-border/70 bg-muted/40 p-2">
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs leading-snug text-muted-foreground">
            ...
            <p className="mt-0.5">
              Create a personal access token with the <code className="font-mono">issue</code> scope
              at <span className="font-medium">Settings → Applications → Access Tokens</span>.
            </p>
          </div>
        </div>
      </div>
